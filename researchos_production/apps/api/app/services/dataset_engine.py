...
def create_dataset_card(name: str, abstract: str, files: list[str], license: str | None, domain: str) -> dict:
    ...
    return {
        ...
        'collection_process': 'Describe sensors/instruments, participants/materials, sampling rate, protocol, and quality control.',
        'metadata_required': ['authors', 'version', 'license', 'collection date', 'schema', 'variables', 'units', 'missing values'],
        'ethical_considerations': 'Document privacy, consent, anonymization, sensitive attributes, and institutional approvals if applicable.',
        'limitations': ['Dataset card generated from limited metadata.', 'Requires manual verification before DOI publication.'],
...
def reproducibility_check(files: list[str], has_readme: bool = False, has_env: bool = False) -> tuple[float, list[str]]:
    ...
    if not has_data:
        score -= 25; issues.append('No obvious data file detected.')
    if not readme:
        score -= 20; issues.append('README is missing.')
    if has_code and not env:
        score -= 20; issues.append('Code is present but environment/dependency file is missing.')
    if not license_file:
    ...
    if not any('citation' in f or 'codemeta' in f for f in lower):
        score -= 10; issues.append('Citation metadata is missing.')
